...
const Popup = ({ onClose }) => {
  ...
  const ConsultarArbitro = async () => {
    const { data } = await axios.get(
      "http://localhost:4000/api/MostrarArbitroTorneo"
    );
    console.log(data);
    setArbitro(data);
  };
  ...
  const ConsultarLocal = async () => {
    const { data } = await axios.get(
      "http://localhost:4000/api/MostrarLocalTorneo"
    );
    console.log(data);
    setLocales(data);
  };
      ...
      <div className="popup-content popup-content-scroll">
        ...
        <form>
          ...
          <div className="form-field">
            ...
            <select
              name="sistemaDesempate"
              id="sistemaDesempate"
              value={body.sistemaDesempate}
              onChange={inputChange}
            >
              ...
              {(body.tipoTorneo === "0" || body.tipoTorneo === "1") &&
                body.formatoJuego === "2" && (
                  <option value="2">Bucholtz</option>
                )}
            </select>
