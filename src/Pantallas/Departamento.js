...
import Popup from "../Popups/PopupDepartamento";
...
export default function Departamento() {
  ...
  const inputChange = async ({ target }) => {
    ...
    const { data } = await axios.post(
      "http://localhost:4000/api/BuscarDepartamento",
      Busqueda
    );
    console.log(data);
    setUserList(data);
  ...
  const Eliminar = async (id) => {
    try {
      const { data } = await axios.post(
        "http://localhost:4000/api/EliminarDepartamento",
        { id: id }
      );
      getUser();
      console.log(data);
  ...
  return (
    <>
      <List2 />
      <h1 className="text-center">Tabla de Departamento</h1>
          ...
          <Table>
            ...
            <TableBody>
              {userList.map((user, index) => (
                <TableRow key={index}>
                  <TableCell>{user.Id_Departamento}</TableCell>
                  ...
                  <TableCell>
                    ...
                    <IconButton
                      onClick={() => Eliminar(user.Id_Departamento)}
                      size="small"
                      color="secundary"
                    >
