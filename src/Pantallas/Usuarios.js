...
import Popup from "../Popups/PopupUsuarios"
...
export default function Usuarios() {
  ...
  const inputChange = async ({ target }) => {
    ...
    const { data } = await axios.post(
      "http://localhost:4000/api/BuscarDeportista",
      Busqueda
    );
    console.log(data);
    setUserList(data);
  ...
  const getUser = async () => {
    const { data } = await axios.get(
      "http://localhost:4000/api/MostrarUsuario"
    );
    console.log(data);
    setUserList(data);
  };
  ...
  const Eliminar = async (id) => {
    try {
      const { data } = await axios.post(
        "http://localhost:4000/api/EliminarUsuario",
        { id: id }
      );
      getUser();
      console.log(data);
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Nombre usuarios</TableCell>
