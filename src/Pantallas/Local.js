...
import {
  ...
  import Popup from "../Popups/PopupLocal";
  ...
  export default function Local() {
    ...
    const inputChange = async ({ target }) => {
      ...
      const { data } = await axios.post(
        "http://localhost:4000/api/BuscarLocal",
        Busqueda
      );
      console.log(data);
      setUserList(data);
    ...
    const getUser = async () => {
      const { data } = await axios.get(
        "http://localhost:4000/api/MostrarLocal"
      );
      console.log(data);
      setUserList(data);
    };
    ...
    const Eliminar = async (id) => {
      try {
        const { data } = await axios.post(
          "http://localhost:4000/api/EliminarLocal",
          { id: id }
        );
        getUser();
        console.log(data);
    ...
    return (
      <>
        <List2 />
        <h1 className="text-center">Tabla de local</h1>
            ...
            <Table>
              ...
              <TableBody>
                {userList.map((user, index) => (
                  <TableRow key={index}>
                    <TableCell>{user.Id_Local}</TableCell>
                    ...
                    <TableCell>
                      ...
                      <IconButton
                        onClick={() => Eliminar(user.Id_Local)}
                        size="small"
                        color="secundary"
                      >
